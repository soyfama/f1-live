...
interface Strategy { id: string; name: string; stints: StintConfig[]; color: string; totalTime: number; pitCount: number; }
interface RealStint { stint_number: number; lap_start: number; lap_end: number; compound: string; }
interface MeetingOption { meetingKey: number; label: string; }
interface SessionOption { sessionKey: number; label: string; }
...
function simulateStrategy(stints: StintConfig[], baseLap: number, pitLoss: number, totalLaps: number, degradation: Record<string, number>): number[] {
...
export default function StrategyClient() {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.openf1.org/v1/drivers?session_key=${selectedSession}`)
      ...
      .then((data: Array<{ driver_number: number; name_acronym: string; full_name: string; team_name: string }>) => {
        const opts: DriverOption[] = data.filter(d => d.name_acronym).map(d => ({ driverNumber: d.driver_number, acronym: d.name_acronym, fullName: d.full_name ?? d.name_acronym, team: d.team_name ?? '' })).sort((a, b) => a.acronym.localeCompare(b.acronym));
        setDrivers(opts);
        if (opts.length > 0) { setSelectedDriver(opts[0].driverNumber); setDriverLabel(`${opts[0].acronym}`); }
      })
      .catch(() => {});
  }, [selectedSession]);
  ...
  const loadStints = useCallback(async () => {
    ...
    finally { setLoadingStints(false); }
  ...
  useEffect(() => {
    const generated: Strategy[] = PRESET_STRATEGIES.map((preset, i) => {
      ...
      return { id: `strat-${i}`, name: preset.name, stints, color: STRATEGY_COLORS[i % STRATEGY_COLORS.length], totalTime: lapTimes[lapTimes.length - 1] ?? 0, pitCount: stints.length - 1 };
    });
  ...
  const { chartData, deltaData } = useMemo(() => {
    ...
    for (const strat of strategies) allLapTimes[strat.id] = simulateStrategy(strat.stints, baseLapTime, pitLoss, totalLaps, degradation);
    ...
    for (let lap = 1; lap <= totalLaps; lap++) {
      ...
      for (const strat of strategies) { const t = allLapTimes[strat.id][lap - 1] ?? 0; if (t > 0 && t < minTime) minTime = t; }
      for (const strat of strategies) { const t = allLapTimes[strat.id][lap - 1] ?? 0; row[strat.id] = t; deltaRow[strat.id] = t > 0 ? parseFloat((t - minTime).toFixed(3)) : 0; }
      chartData.push(row);
  ...
  const bestStrategy = useMemo(() => strategies.reduce((best, s) => (!best || s.totalTime < best.totalTime) ? s : best, strategies[0]), [strategies]);
