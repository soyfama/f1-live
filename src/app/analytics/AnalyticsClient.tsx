...
export default function AnalyticsClient() {
  ...
  return (
    <div className="flex gap-0 min-h-[calc(100vh-3.5rem)]">
      <aside className="w-48 shrink-0 border-r border-[#1a1a1a] bg-[#0a0a0a] py-5 px-3">
        <p className="text-gray-600 text-[10px] uppercase tracking-widest px-2 mb-3 font-bold">Analysis</p>
        <nav className="space-y-1">
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-all ${activeTab === tab.id ? 'bg-[#E10600]/15 text-white font-semibold border border-[#E10600]/30' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>
      ...
      <div className="flex-1 px-6 py-5 overflow-auto">
        <div className="mb-5 border-b border-[#1a1a1a] pb-4">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <select value={year} onChange={e => setYear(Number(e.target.value))} className="bg-[#111] border border-[#222] text-white rounded-lg px-3 py-2 text-sm">
              {[2026, 2025, 2024, 2023].map(y => <option key={y} value={y}>{y}</option>)}
            </select>
            <select value={selectedMeeting ?? ''} onChange={e => setSelectedMeeting(Number(e.target.value))} className="bg-[#111] border border-[#222] text-white rounded-lg px-3 py-2 text-sm min-w-[200px]">
              {meetings.map(m => <option key={m.meetingKey} value={m.meetingKey}>{m.label}</option>)}
            </select>
            <select value={selectedSession ?? ''} onChange={e => setSelectedSession(Number(e.target.value))} className="bg-[#111] border border-[#222] text-white rounded-lg px-3 py-2 text-sm">
              {sessions.map(s => <option key={s.sessionKey} value={s.sessionKey}>{s.label}</option>)}
            </select>
            <button onClick={loadData} disabled={loading} className="bg-[#E10600] hover:bg-[#c00500] disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">{loading ? 'Loading...' : 'Load'}</button>
          </div>
        </div>
          ...
          <>
                  ...
                  {driverChartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height={Math.max(400, driverChartData.length * 32)}>
                      <BarChart data={driverChartData} layout="vertical" margin={{ left: 10, right: 80 }} barSize={18}>
                        ...
                        <XAxis type="number" tickFormatter={(v) => formatLapTime(v)} tick={{ fill: '#555', fontSize: 11, fontFamily: 'monospace' }} domain={[(dataMin: number) => Math.floor(dataMin * 0.997), (dataMax: number) => Math.ceil(dataMax * 1.001)]} />
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-gray-600">
                      ...
                      <p className="text-sm font-medium text-gray-500">No lap data available</p>
                          ...
                          {driverChartData.map((d, i) => {
                            ...
                            return (
                              <tr key={d.name} className="border-b border-[#1a1a1a]/50 hover:bg-white/[0.02]">
                                ...
                                <td className="py-3 px-3 text-gray-500 text-xs">{d.team?.replace(' F1 Team', '').replace('Oracle Red Bull Racing', 'Red Bull')}</td>
                                ...
                                <td className="py-3 px-3 text-right font-mono text-gray-500 hidden md:table-cell text-xs">{d.s1 ? d.s1.toFixed(3) : '—'}</td>
                                <td className="py-3 px-3 text-right font-mono text-gray-500 hidden md:table-cell text-xs">{d.s2 ? d.s2.toFixed(3) : '—'}</td>
                                <td className="py-3 px-3 text-right font-mono text-gray-500 hidden md:table-cell text-xs">{d.s3 ? d.s3.toFixed(3) : '—'}</td>
                                <td className="py-3 px-3 text-right text-gray-600 text-xs">{d.laps}</td>
                              </tr>
                            );
                          })}
                        </tbody>
              ...
              <div className="chart-container">
                ...
                {teamChartData.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-16 text-gray-600">
                    ...
                    <p className="text-sm font-medium text-gray-500">No team data available</p>
                    <p className="text-xs mt-1">Select a session and click Load</p>
                  </div>
                )}
              </div>
                    ...
                    {analyticsData.stints.map((s, i) => {
                      ...
                      return (
                        <div key={i} className="bg-[#0a0a0a] rounded-lg p-3 text-xs border border-[#1a1a1a]">
                          ...
                          <p className="text-gray-500">{s.laps} laps • {s.compound}</p>
                          <p className="text-gray-600 font-mono">L{s.from} → L{s.to}</p>
                        </div>
                      );
                    })}
                  </div>
                ) : <p className="text-gray-600 text-center py-12">No long stints found</p>}
              </div>
            )}
              ...
              <div className="chart-container space-y-6">
                ...
                {analyticsData.sectors?.length ? (
                  ['S1', 'S2', 'S3'].map((sector, si) => (
                    <div key={sector}>
                      <h4 className="text-gray-500 text-sm mb-2 font-medium">Sector {si + 1}</h4>
                      <ResponsiveContainer width="100%" height={220}>
                        <BarChart data={analyticsData.sectors.slice(0, 15)} layout="vertical" margin={{ left: 10, right: 60 }} barSize={14}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#1a1a1a" horizontal={false} />
                          <XAxis type="number" tickFormatter={(v) => v.toFixed(3)} tick={{ fill: '#555', fontSize: 10, fontFamily: 'monospace' }} domain={[(dataMin: number) => Math.floor(dataMin * 0.997), (dataMax: number) => Math.ceil(dataMax * 1.001)]} />
                ...
                ) : <p className="text-gray-600 text-center py-12">No sector data</p>}
              </div>
            )}
            ...
            {activeTab === 'speed' && (
              <div className="chart-container">
                      ...
                      <tbody>
                        {analyticsData.speed.map((d, i) => (
                          <tr key={i} className="border-b border-[#1a1a1a]/50 hover:bg-white/[0.02]">
                            <td className="py-3 px-3 text-gray-500 font-mono">{i + 1}</td>
                            <td className="py-3 px-3 font-bold text-white">{d.name}</td>
                            <td className="py-3 px-3 text-gray-500 text-xs">{d.team?.replace(' F1 Team', '')}</td>
                            <td className="py-3 px-3 text-right font-mono text-[#00FF00]">{d.speed}</td>
                            <td className="py-3 px-3">
                ...
                ) : <p className="text-gray-600 text-center py-12">No speed data</p>}
              </div>
            )}
          </>
        )}
