        ...
        <div className="max-w-7xl mx-auto px-4 py-12 md:py-16">
          ...
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-3 mb-6">
              <span className="text-[#E10600] font-black text-6xl md:text-7xl tracking-tighter">F1</span>
              <span className="text-white font-bold text-6xl md:text-7xl tracking-tighter">Live</span>
              <div className="w-3 h-3 rounded-full bg-[#E10600] animate-pulse-dot ml-2 mt-2" />
            </div>
          ...
          {/* Live Banner */}
          {loading ? (
            <div className="f1-card p-6 mb-8 animate-pulse">
              <div className="h-24 bg-white/5 rounded-lg skeleton-shimmer" />
            </div>
          ) : liveData?.session ? (
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full bg-[#E10600] animate-pulse-dot" />
                        <span className="text-[#E10600] font-bold text-sm tracking-widest uppercase">Live Now</span>
                      </div>
                      <div className="w-px h-4 bg-white/10" />
                      <div>
                        <span className="text-white font-bold text-lg">{liveData.session.year} {liveData.session.country} GP</span>
                        <span className="text-gray-500 text-sm ml-2">• {liveData.session.circuit}</span>
                      </div>
                    </div>
                    <span className="text-2xl font-black text-white/10 uppercase tracking-widest">
                  ...
                  <div className="grid grid-cols-5 gap-2 md:gap-4">
                    {liveData.drivers.map((driver, idx) => {
                      ...
                      return (
                        <div 
                          key={driver.driver}
                          className={`relative rounded-xl p-4 transition-all duration-200 hover:scale-105 ${
                            idx === 0 ? 'podium-1' : 
                            idx === 1 ? 'podium-2' : 
                            idx === 2 ? 'podium-3' : 
                        ...
                        >
                          ...
                          <div className="text-center">
                            <div className={`font-black text-3xl md:text-4xl mb-1 ${
                              idx === 0 ? 'text-[#FFD700]' : 
                              idx === 1 ? 'text-[#C0C0C0]' : 
                              idx === 2 ? 'text-[#CD7F32]' : 
                              'text-gray-500'
                            ...
                            <div className="font-bold text-white text-lg">{driver.driver}</div>
                            <div className="text-xs text-gray-500 uppercase tracking-wider truncate">{driver.team?.split(' ')[0]}</div>
                            ...
                            {driver.best_lap && driver.best_lap !== '--' && (
                              <div className="mt-1 text-[10px] mono text-[#BF00FF]">{driver.best_lap}</div>
                            )}
                          </div>
                        </div>
                      );
            ...
            <div className="f1-card p-6 md:p-8 mb-8">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs text-gray-500 uppercase tracking-widest">Next Race</span>
                  </div>
                  <h2 className="text-2xl font-bold text-white mb-1">{nextRace.name}</h2>
                  <p className="text-gray-400">{nextRace.circuit} • {nextRace.country}</p>
                </div>
                <div className="text-right">
                  <div className="text-4xl md:text-5xl font-black text-[#E10600] mono tracking-tight">
                    {nextRace.countdown}
                  </div>
                  <p className="text-xs text-gray-500 uppercase tracking-wider mt-1">Until Race Start</p>
                </div>
              </div>
            </div>
          ) : null}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {FEATURES.map((feature) => (
            <Link
              key={feature.href}
              href={feature.href}
              className="group f1-card p-5 hover:border-[#E10600]/30 transition-all duration-200"
            >
              <div className="flex items-start justify-between mb-4">
                <span className="text-2xl font-bold" style={{ color: feature.color }}>{feature.icon}</span>
                <span className="text-[10px] text-gray-600 uppercase tracking-wider group-hover:text-[#E10600] transition-colors">
                  View →
                </span>
              </div>
      ...
      <div className="max-w-6xl mx-auto px-4 pb-8">
        <div className="border-t border-white/5 pt-6 text-center">
          <p className="text-xs text-gray-600">
            Data provided by{' '}
            <a href="https://openf1.org" target="_blank" rel="noopener" className="text-gray-400 hover:text-white transition-colors underline">
              OpenF1
            </a>
